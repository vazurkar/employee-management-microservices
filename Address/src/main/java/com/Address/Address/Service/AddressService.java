...
import com.Address.Address.Exception.ServiceNotAvailableException;
...
public class AddressService {
    ...
    @CircuitBreaker(
            ...
            fallbackMethod = "addressServiceFallback"
    )
    public List<AddressDTO> addAddress(AddressRequest addressRequest) {
        ...
        List<Address> listToSave =
                this.saveOrUpdate(addressRequest);

        List<Address> savedAddress =
                addressRepository.saveAll(listToSave);
    ...
    public List<AddressDTO> updateAddress(
        ...
        List<Address> addressbyEmpID =
                addressRepository.findAllByEmpId(
                        addressRequest.getEmpId()
                );
        ...
        if (addressbyEmpID.isEmpty()) {
            ...
            log.info(
                    "No address found with emp id "
                            + addressRequest.getEmpId()
            );

            log.info(
                    "creating new address with emp id "
                            + addressRequest.getEmpId()
            );
        }
    ...
    private List<Address> saveOrUpdate(
            AddressRequest addressRequest) {

        List<Address> listToSave = new ArrayList<>();

        for (AddressRequestDTO addressRequestDTO :
                addressRequest.getAddresses()) {
            ...
            address.setAddressType(
                    addressRequestDTO.getAddressType()
            );
    ...
    public List<AddressDTO> addressServiceFallback(
            AddressRequest addressRequest,
            Exception ex) {
        ...
        throw new ServiceNotAvailableException(
                "Employee Service is unavailable. Please try again later.", HttpStatus.SERVICE_UNAVAILABLE
        );
    }
}